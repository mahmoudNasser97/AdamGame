...
namespace Opsive.UltimateCharacterController.Input
{
    ...
    public abstract class PlayerInput : StateBehavior
    {
        ...
        [Tooltip("The name of the horizontal camera input mapping.")]
        [SerializeField] protected string m_ControllerHorizontalLookInputName = "PS4_RightStickHorizontal";
        [Tooltip("The name of the vertical camera input mapping.")]
        [SerializeField] protected string m_ControllerVerticalLookInputName = "PS4_RightStickVertical";
        ...
        private void FixedUpdate()
        {
            ...
            if (IsContConnected())
            {
                m_RawLookVector.x = Input.GetAxisRaw(m_ControllerHorizontalLookInputName) + GetAxisRaw(m_HorizontalLookInputName);
                m_RawLookVector.y = -Input.GetAxisRaw(m_ControllerVerticalLookInputName) + GetAxisRaw(m_VerticalLookInputName);
            }
            else
            {
            ...
            else if (m_LookVectorMode == LookVectorMode.UnitySmoothed)
            {
                if (IsContConnected())
                {
                    m_CurrentLookVector.x = Input.GetAxis(m_ControllerHorizontalLookInputName) + GetAxis(m_HorizontalLookInputName);
                    m_CurrentLookVector.y = -Input.GetAxis(m_ControllerVerticalLookInputName) + GetAxis(m_VerticalLookInputName);
                }
                else
                {
        ...
        public void ChangeAxisName(string horizontal, string vertical)
        {
            if (!IsContConnected())
                return;

            m_ControllerHorizontalLookInputName = horizontal;
            m_ControllerVerticalLookInputName = vertical;
        }
    }
}