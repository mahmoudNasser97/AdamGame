...
namespace Opsive.UltimateCharacterController.Utility
{
    ...
    public class MathUtility
    {
        ...
        {
            // Recursively multiply the matrices as long as the current Transform is not at the root.0
            if (current != root) {
                return ApplyRotationToChildMatrices(current.parent, root, deltaRotation) * Matrix4x4.TRS(current.localPosition, current.localRotation.normalized, current.localScale);// made Quartenion value normalized to ignore null values during Quartenion to Matrix Conversion(Gaurav)
            }
            // At the root of the tree, apply the delta to the rotation and return the matrix.
            return Matrix4x4.TRS(current.localPosition, current.localRotation.normalized , current.localScale);// made Quartenion value normalized to ignore null values during Quartenion to Matrix Conversion(Gaurav)
        }
