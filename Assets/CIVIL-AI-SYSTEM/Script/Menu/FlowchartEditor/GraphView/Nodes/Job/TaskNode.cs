...
{
    public class TaskNode : AISystem.Flowchart.V2.Nodes.Common.Node
    {
        new DutyTask data;

        public TaskNode(GraphViewFlowchart graphView, Vector2 position) : base(graphView, position, "Task")
        {
            data = new DutyTask("New Task", "", null, null);
            Setup();
        }

        public TaskNode(GraphViewFlowchart graphView, Vector2 position, DutyTask dataRef) : base(graphView, position, "Task", dataRef)
        {
            data = dataRef;
            Setup();
        }
        ...
        {
            AISystem.Civil.Objects.V2.DutyTask typedData = (AISystem.Civil.Objects.V2.DutyTask)data;
